...
export const MultiImageUpload: React.FC<MultiImageUploadProps> = ({
    ...
    async (slot: number, file: File) => {
      ...
      if (file.size > 10 * 1024 * 1024) {
        ...
        const msgId = MessagePlugin.loading({
          content: `正在压缩图片 (${(file.size / 1024 / 1024).toFixed(
            1
          )}MB)...`,
          duration: 0,
          placement: 'top',
        ...
        try {
          fileToProcess = (await compressImageBlob(
            file,
            strategy.targetSizeMB
          )) as File;
          MessagePlugin.close(msgId);
          MessagePlugin.success({
            content: `压缩完成: ${(file.size / 1024 / 1024).toFixed(1)}MB → ${(
              fileToProcess.size /
              1024 /
              1024
            ).toFixed(1)}MB`,
          ...
          MessagePlugin.close(msgId);
          console.error(
            '[MultiImageUpload] Compression failed:',
            compressionErr
          );
  ...
  const handleMediaLibrarySelect = useCallback(
    async (asset: Asset) => {
      try {
        ...
        console.error(
          '[MultiImageUpload] Failed to convert asset to base64:',
          error
        );
  ...
  const assetToUploadedImage = useCallback(
    async (asset: Asset, slot: number): Promise<UploadedVideoImage> => {
  ...
  const upsertImagesBySlot = useCallback(
    (baseImages: UploadedVideoImage[], newImages: UploadedVideoImage[]) => {
      const mergedImages = [...baseImages];

      for (const newImage of newImages) {
        const existingIndex = mergedImages.findIndex(
          (img) => img.slot === newImage.slot
        );
        if (existingIndex >= 0) {
  ...
  const handleDesktopUpload = useCallback(
    async (slot: number) => {
      ...
      try {
        ...
        const newImage = await assetToUploadedImage(asset, slot);
        onImagesChange(upsertImagesBySlot(images, [newImage]));
      } catch (error) {
        console.error(
          '[MultiImageUpload] Desktop local selection failed:',
          error
        );
        MessagePlugin.error('加载图片失败');
      }
    },
    [assetToUploadedImage, images, onImagesChange, upsertImagesBySlot]
  );
  ...
  const handleMediaLibrarySelectMultiple = useCallback(
    async (assets: Asset[]) => {
      ...
      try {
        ...
        const targetSlots = [
          currentSlot,
          ...remainingSlots.filter((slot) => slot !== currentSlot),
        ];
        ...
        const newImages = await Promise.all(
          assignableAssets.map((asset, index) =>
            assetToUploadedImage(asset, targetSlots[index])
          )
        );
        ...
        if (assets.length > targetSlots.length) {
          MessagePlugin.warning(
            `最多可批量使用 ${targetSlots.length} 张图片，已自动截取`
          );
        }
      } catch (error) {
        console.error(
          '[MultiImageUpload] Failed to batch convert assets:',
          error
        );
        MessagePlugin.error('加载图片失败');
        setShowMediaLibrary(false);
      ...
      <div key={slot} className="multi-image-upload__slot">
        ...
        {image ? (
          ...
          <div className="multi-image-upload__placeholder-container">
            {isTauriEnvironment() ? (
              <div
                className="multi-image-upload__placeholder"
                role="button"
                aria-disabled={disabled}
                onClick={() => {
                  if (!disabled) {
                    handleDesktopUpload(slot);
                  }
                }}
              >
                <ImageUploadIcon
                  size={24}
                  className="multi-image-upload__add-icon"
                />
                <span className="multi-image-upload__add-text">
                  上传{label}
                </span>
              </div>
            ) : (
