...
export const ReferenceImageUpload: React.FC<ReferenceImageUploadProps> = ({
  ...
  const handleDesktopLocalSelect = useCallback(
    async (targetSlot?: number) => {
      ...
      try {
        ...
        let imageFiles = pickedFiles.filter(
          (file) =>
        ...
        if (slotLabels) {
          const startSlot = targetSlot ?? currentSlot;
          const availableSlots = slotLabels.length - startSlot;
          imageFiles = imageFiles.slice(0, Math.max(availableSlots, 0));
        } else if (multiple) {
          imageFiles = imageFiles.slice(
            0,
            Math.max(maxCount - images.length, 0)
          );
        } else {
        ...
        if (slotLabels) {
          applySlotImages(nextImages, targetSlot ?? currentSlot);
        } else if (multiple) {
          onImagesChange([...images, ...nextImages]);
        } else {
          onImagesChange(nextImages.slice(0, 1));
        }
        onError?.(null);
        return true;
      } catch (error) {
        console.error(
          '[ReferenceImageUpload] Desktop local selection failed:',
          error
        );
        onError?.(t.loadFailed);
        return true;
  ...
  const openFileDialog = useCallback(
    async (slot?: number) => {
      ...
      const handled = await handleDesktopLocalSelect(slot);
      if (handled) return;
      fileInputRef.current?.click();
    },
    [handleDesktopLocalSelect]
  );
